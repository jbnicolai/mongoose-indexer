    ...
    async.waterfall([
        ...
        function loadFiles(filenames, callback) {
            ...
            for (var i=0; i<filenames.length; i++){
                ...
                if(path.resolve(filenames[i]) == path.normalize(filenames[i])){
                    modelPath =  filenames[i];
                } else {
                    modelPath =   process.cwd() + '/' + filenames[i];
                }
                
                if (!options.passMongooseObject){
